...
const getDateCondition = (period, startDate, endDate) => {
    if (period) {
        switch (period) {
            case 'today':     return ' AND DATE(o.created_at) = CURDATE()';
            case 'yesterday': return ' AND DATE(o.created_at) = DATE_SUB(CURDATE(), INTERVAL 1 DAY)';
            case 'week':      return ' AND YEARWEEK(o.created_at, 1) = YEARWEEK(CURDATE(), 1)';
            case 'month':     return ' AND YEAR(o.created_at) = YEAR(CURDATE()) AND MONTH(o.created_at) = MONTH(CURDATE())';
            case 'year':      return ' AND YEAR(o.created_at) = YEAR(CURDATE())';
        }
    } else if (startDate && endDate) {
        return ` AND DATE(o.created_at) BETWEEN '${startDate}' AND '${endDate}'`;
    }
    return '';
};
    ...
    try {
        ...
        const [report] = await promisePool.query(
            `SELECT
             ...
             FROM orders
             WHERE DATE(created_at) = ? AND merchant_id = ? AND deleted_at IS NULL`,
            [reportDate, merchantId]
        );
        ...
        const [report] = await promisePool.query(
            `SELECT
             ...
             WHERE DATE(created_at) BETWEEN ? AND ?
               AND status = 'completed' AND merchant_id = ? AND deleted_at IS NULL
             GROUP BY DATE(created_at)
             ORDER BY DATE(created_at) ASC`,
            [start_date, end_date, merchantId]
        );
...
const getTopSellingProducts = async (req, res) => {
    try {
        ...
        let query = `
            SELECT
                p.id, p.name, p.image_emoji, c.name as category_name,
            ...
            WHERE o.status = 'completed' AND o.merchant_id = ? AND o.deleted_at IS NULL
        `;
        ...
        query += ' GROUP BY p.id, p.name, p.image_emoji, c.name ORDER BY total_revenue DESC LIMIT ?';
    ...
    try {
        ...
        const [report] = await promisePool.query(
            `SELECT payment_method, COUNT(*) as order_count, SUM(total) as total_amount
             FROM orders
             WHERE DATE(created_at) = ? AND status = 'completed' AND merchant_id = ? AND deleted_at IS NULL
...
const getLowStockProducts = async (req, res) => {
    try {
        ...
        const [products] = await promisePool.query(
            `SELECT p.id, p.name, p.stock_quantity, p.low_stock_threshold, p.price,
                    c.name as category_name, c.icon as category_icon
             FROM products p
             INNER JOIN categories c ON p.category_id = c.id
             WHERE p.stock_quantity <= p.low_stock_threshold
               AND p.is_active = TRUE AND p.merchant_id = ? AND p.deleted_at IS NULL
...
const getProductSalesStats = async (req, res) => {
    try {
        ...
        const [stats] = await promisePool.query(
            `SELECT
                p.id as product_id, p.name as product_name, p.image_emoji, c.name as category_name,
        ...
        if (stats.length === 0) return sendError(res, 'Product not found', 404);
...
const getSalesSummary = async (req, res) => {
    try {
        ...
        const [currentStats] = await promisePool.query(
            ...
            WHERE o.status = 'completed' AND o.deleted_at IS NULL AND o.merchant_id = ?
            ${dateCondition}`,
            [merchantId]
        );
...
const getTopProducts = async (req, res) => {
    try {
        ...
        const [products] = await promisePool.query(
            `SELECT
                p.id as product_id, p.name as product_name, p.image_emoji,
        ...
        const [categories] = await promisePool.query(
            `SELECT
                ...
                (COALESCE(SUM(oi.quantity * oi.unit_price), 0) /
                    NULLIF((SELECT SUM(quantity * unit_price)
                     FROM order_items oi2
                     INNER JOIN orders o2 ON oi2.order_id = o2.id
                     WHERE o2.status = 'completed' AND o2.deleted_at IS NULL AND o2.merchant_id = ? ${dateCondition}
                    ), 0) * 100
...
const getRecentOrders = async (req, res) => {
    try {
        ...
        const merchantId = req.user.merchant_id;
        const dateCondition = getDateCondition(period, startDate, endDate);
        const statusCondition = (status && status !== 'all') ? ` AND o.status = '${status}'` : '';

        const [orders] = await promisePool.query(
            `SELECT
                o.id, o.order_number as code, o.table_id, o.status, o.payment_method,
                o.subtotal, o.tax_amount, o.total as grand_total, o.created_at as date,
...
const exportReport = async (req, res) => {
    try {
        ...
        sendSuccess(res, { message: `${format.toUpperCase()} export will be implemented soon`, format }, 'Export initiated');
    } catch (error) {
